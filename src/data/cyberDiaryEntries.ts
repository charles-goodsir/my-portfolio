...
export const cyberDiaryEntries: DiaryEntry[] = [
  {
    ...
    title:
      'AppSec Homelab Entry 8: Docker deployment and the first ZAP baseline scan',
    workedOn: [
      ...
      'Ran the first OWASP ZAP baseline scan against the deployed app, which completes the automated set alongside Semgrep (SAST) and gitleaks (secrets)',
    ...
    body: [
      'Picking up from yesterday. The plan for today: move the mini PC to a lighter headless boot, get a GitHub key on the box that is independent of my Mac, get the vulnerable app running in Docker on it, and run a first ZAP scan against it.',
      'First, the boot target. sudo systemctl set-default multi-user.target && sudo reboot drops the desktop environment on boot and frees the RAM it was holding, which I want back for Docker and ZAP. The thing to check afterwards was SSH, since there is no desktop to fall back on now if it does not come up. It reconnected cleanly.',
      ...
      'Then the firewall rule to reach it: sudo ufw allow from 192.168.88.0/24 to any port 8080 proto tcp. I scoped it to my home subnet rather than allowing 8080 from anywhere. The app is intentionally vulnerable, so there is no reason for that port to be reachable from outside the local network, even by accident. It is a small thing but the kind of default worth keeping visible in the writeup.',
      'With the app up, I ran the first ZAP baseline scan: docker run -t -v $(pwd):/zap/wrk/:rw zaproxy/zap-stable zap-baseline.py -t http://192.168.88.13:8080 -r zap-report.html. That is the DAST layer, and the first time the three automated checks run end to end. Semgrep already covers SAST and gitleaks covers secrets.',
