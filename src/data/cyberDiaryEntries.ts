...
export const cyberDiaryEntries: DiaryEntry[] = [
  {
    ...
    workedOn: [
      ...
      'Fixed that with unzip -o, then hit a second failure straight after: cannot delete old terraform: Is a directory, because a stray terraform directory from the failed run was left in the workspace and unzip can overwrite a file but not a directory',
      ...
      'Caught a YAML formatting mistake along the way: a pasted curl command with an accidental line break split into two broken shell commands, since line breaks in a script: | block are real command boundaries unless escaped with \\',
    ...
    body: [
      ...
      'Hit the first real bug straight away: the pipeline hung indefinitely on the Install Terraform step. Turned out unzip was silently waiting on an interactive overwrite prompt (replace LICENSE.txt? [y/n]) left over from a re-run, and a hosted agent has no stdin to answer it, so the job just sat there until timeout. Fixed by adding -o to force a non-interactive overwrite.',
      'The very next run failed differently: cannot delete old terraform: Is a directory, followed by terraform: command not found. Root cause was a previous failed run leaving behind a stray directory called terraform in the workspace. unzip -o can force-overwrite a file, but it has no equivalent for a directory sitting in the way, so it failed silently on the delete and left no terraform binary behind.',
      'Fixed it properly by not trusting the workspace to be clean at all: added rm -rf terraform terraform.zip to the very top of the script, before the curl and unzip steps run. On a hosted agent that gets reused across pipeline retries, a clean slate has to be enforced, not assumed.',
      ...
      'With all three fixes in, terraform -version now prints Terraform v1.9.8 cleanly on every run, no matter what state a previous run left the workspace in. Small step, but it is the foundation the rest of the Validate stage builds on.',
