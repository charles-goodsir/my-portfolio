...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === '/visit' && request.method === 'POST') {
      ...
      await Promise.all([
        increment(env.VISITS, 'visits:total'),
        increment(env.VISITS, `visits:country:${country}`),
      ])
    ...
    if (url.pathname === '/stats' && request.method === 'GET') {
      ...
      const total = parseInt(
        (await env.VISITS.get('visits:total')) ?? '0',
        10,
      )
      const list = await env.VISITS.list({ prefix: 'visits:country:' })
      const byCountry: Record<string, number> = {}
      for (const key of list.keys) {
        const code = key.name.replace('visits:country:', '')
